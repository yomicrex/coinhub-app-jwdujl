...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      // CRITICAL: Clear any cached user data before fetching
      setUser(null);
  ...
  const signIn = async (email: string, password: string) => {
    ...
    try {
      // CRITICAL: Clear any existing user data before signing in
      console.log('AuthContext: SignIn - Clearing existing user data');
      setUser(null);
      ...
      // CRITICAL: Fetch the full user profile directly from the backend
      // This ensures we get the correct profile for the logged-in user
      console.log('AuthContext: SignIn - Fetching user profile after sign in');
      ...
      // CRITICAL: Clear user data on error
      setUser(null);
  ...
  const signUp = async (email: string, password: string) => {
    ...
    try {
      // CRITICAL: Clear any existing user data before signing up
      console.log('AuthContext: SignUp - Clearing existing user data');
      setUser(null);
      ...
      console.log('AuthContext: SignUp - Fetching user profile after sign up');
      const userData = await fetchUserProfile();
  ...
  const signOut = async () => {
    ...
    try {
      // CRITICAL: Clear user data BEFORE signing out
      console.log('AuthContext: SignOut - Clearing user data');
      setUser(null);
      ...
      // Still clear user data even if signNow let me also fix the frontend to ensure it's properly handling the user profile data and not caching incorrectly:
